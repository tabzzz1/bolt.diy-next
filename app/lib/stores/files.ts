...
import { STORAGE_KEY_DELETED_PATHS } from '~/lib/persistence/storageKeys';
...
export class FilesStore {
  ...
  constructor(webcontainerPromise: Promise<WebContainer>) {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        const deletedPathsJson = localStorage.getItem(STORAGE_KEY_DELETED_PATHS);
  ...
  #persistDeletedPaths() {
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem(STORAGE_KEY_DELETED_PATHS, JSON.stringify([...this.#deletedPaths]));
      }
    } catch (error) {
